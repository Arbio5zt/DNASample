...
func sendSampleTransaction(cmd map[string]interface{}) map[string]interface{} {
	...
	case "contract1":
		...
		fcd := &code.FunctionCode{
			...
			ReturnTypes:    []contract.ContractParameterType{},
		}
		hash := fcd.CodeHash()
	...
	case "contract2":
		...
		return responsePacking(fmt.Sprintf("pubHash: %x, invHash: %x", pubHash, invHash), id)
	case "contract3":
		//calc if(a>b) => a* sum(b,c) else => sum(a,b) *c publish
		...
		fcd := &code.FunctionCode{
			Code:           rcode,
			ParameterTypes: []contract.ContractParameterType{contract.Integer, contract.Integer, contract.Integer},
			ReturnTypes:    []contract.ContractParameterType{contract.Integer},
		}
		hash := fcd.CodeHash()
		codeHash_H := hash.ToArray()
		...
		//if(a>b) => a* sum(b,c) else => sum(a,b) *c -invoke
		time.Sleep(5 * time.Second)
		log.Fatal("Transaction start.")
		//Inovke SmartContract Return "H"
		...
		return responsePacking(fmt.Sprintf("pubHash: %x, invHash: %x", pubHash, invHash), id)
	case "contract4":
		//save '1' to key 'H'  -publish
		str := "746b615101480568656c6c6f6152726815416e745368617265732e53746f726167652e50757461616c7566"
		rcode, _ := HexToBytes(str)
		fcd := &code.FunctionCode{
			Code:           rcode,
			ParameterTypes: []contract.ContractParameterType{},
